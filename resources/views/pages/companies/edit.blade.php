    ...
    <div class="bg-white p-4 rounded-lg shadow-lg mx-auto text-gray-900">
        ...
        <form action="{{ route('companies.update', $company) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <x-text-input type="text" name="name" id="name" value="{{ old('name') ?? $company->name }}" required/>
                <x-input-error class="mt-2" :messages="$errors->get('name')" />
            </div>

            <div class="mb-4">
                <x-input-label for="cnpj"  :value="__('CNPJ:')"/>
                <x-text-input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj') ?? $company->cnpj }}" required/>
